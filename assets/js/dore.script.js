...
$.dore = function (element, options) {
  ...
  function init() {
    ...
    if (typeof Chart !== "undefined") {
      ...
      if (document.getElementById("salesChart")) {
        ...
        var myChart = new Chart(salesChart, {
          ...
          data: {
            ...
            datasets: [
              {
                label: "Month Wise Collection In Lakhs",
                data: [25, 12, 20, 34, 46, 42],
                borderColor: themeColor1,
                pointBackgroundColor: foregroundColor,
                pointBorderColor: themeColor1,
                pointHoverBackgroundColor: themeColor1,
      ...
      if (document.getElementById("newproductChart")) {
        var productChart = document
          .getElementById("newproductChart")
          .getContext("2d");
    ...
    /*if ($().dropzone && !$(".dropzone").hasClass("disabled")) {
        ...
        url: "http://localhost/aicpe/index.php/institutions/upload_dropzone_file",
            paramName: "file",
            maxFilesize:10,
            maxFiles: 10,
    ...
    if (typeof ClassicEditor !== "undefined") {
      //ClassicEditor.create(document.querySelector("#editor")).catch(function (error) { console.error( error ); });
      //ClassicEditor.create(document.querySelector("#ckEditorClassic")).catch(function (error) { console.error( error ); });
      ClassicEditor.create(document.querySelector("#ckEditorClassic01")).catch(function (error) { });
    }
    ...
    if ($().smartWizard) {
      ...
      $("#smartWizardValidation .finish-btn").on("click", function (event) {
        if (checkWizardValidation($('#smartWizardValidation #form-step-2'))) {
          var dataString = $("#form-step-0,#form-step-1,#form-step-2").serialize();
          var form =  $("#form-step-1")[0];
          var formdata = new FormData(form);

          alert(formdata);
          var base_url = $('#base_url').val();
          $.ajax({
            url: base_url+'add-student',
            type: 'post',
            data: formdata,
            success: function(response){
              if(response.status=='success'){
               toastr.success('New student has been added successfully','Success',{timeOut:5000});
               $( '#form-step-0' ).each(function(){
                        this.reset();
                    });
    ...
    if ($().validate) {
      ...
      $(' .tags-input, .datepicker').on('change', function () {
        $(this).valid();
      });
    }
  }
