...
class Prints extends MY_Controller
{
        ...
        if (is_array($order_ids) && count($order_ids) > 0) {
            ...
            if (count($data['data']) > 0) {
                ...
                if (isset($data['group_on']['print_data'])) {
                    ...
                    if ($action == 'invoice') {
                        $data['manual']['order_terms'] = 'Please notify TNT Deals, Inc. of any damaged product, shortages or overages within 7 days of deliver.';
    ...
    {
        pr($this->input->get('ids'));
       $product_data = $this->Institution_model->get_product_detail_for_label();
    ...
    /*
    @Description    : Function to print manual order BOL from browser
    @Author         : Bhavesh Chaudhari
    @Input          : Order ID, Action from and number of print copy
    @Output         : HTML page of invoice
    @Date           : 16-03-2021
    */
    ...
    public function print_system_bol($id = array(), $action = 'invoice', $print_copy = '1', $test_call = '0')
    {
        $order_id = array();
        $html      = '';
     
        $order_id =   $this->get_request_params('id');
        $order_id =   !is_array($order_id) ? explode(',', $order_id) : $order_id;

        if (count($order_id) > 0 && $order_id!= '') 
        {
            $data['order_id'] = $order_id;

            $order_detail = $this->orders_model->get_bol_related_order_detail($order_id);
        
            if (!empty($order_detail)) 
            {
                $data['ship_from'] = get_company_address_bol();

                $data['order_detail'] = $order_detail;

                echo $this->load->view('order/bill_of_landing', $data,TRUE);
