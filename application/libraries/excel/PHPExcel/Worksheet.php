...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
    ...
    */
    ...
    public $CustomerFormat = null;

    public $column_custom_format = null;
    ...
    */
    ...
    public function fromArray($source = null, $nullValue = null, $startCell = 'A1', $strictNullComparison = false)
    {
        if (is_array($source))
        {
            ...
            // Loop through $source
            foreach ($source as $source_key => $rowData)
            {
                //pr($source);
                $start_column = $currentColumn = $startColumn;
                $end_column = $start_column;
                $CustomerFormat = null;
                if(isset($rowData["CustomerFormat"]))
                {
                    $CustomerFormat = $rowData["CustomerFormat"];

                    unset($rowData["CustomerFormat"]);    
                }
                


                foreach($rowData as $row_key => $cellValue)
                {
                    ...
                    } else
                    {
                        if ($cellValue != $nullValue)
                        {
                            ...
                            {
                                if($this->CustomerFormat && is_array($this->CustomerFormat))
                                {
                                    if(isset($this->CustomerFormat["Value_Format"][$row_key]))
                                    {
                                        if(is_callable($this->CustomerFormat["Value_Format"][$row_key]))
                                        {
                                            $this->CustomerFormat["Value_Format"][$row_key]($cellValue,$currentColumn,$startRow);
                                        }
                                    }
                                }
                            }
                            elseif(isset($this->CustomerFormat["Value_Format"][$row_key]) && is_callable($this->CustomerFormat["Value_Format"][$row_key]))
                            {
                                $this->CustomerFormat["Value_Format"][$row_key]($cellValue,$currentColumn,$startRow,$this);
                            }
                            else
                            {
                                // Set cell value
                            ...
                            if($this->CustomerFormat && is_array($this->CustomerFormat))
                            {
                                ...
                                if(isset($this->CustomerFormat["Number_Format"][$row_key]))
                                {
                                    foreach ($this->CustomerFormat["Number_Format"][$row_key] as $format_key => $format_value)
                                    {
                                        $this->getStyle($currentColumn . $startRow)->getNumberFormat()->setFormatCode($format_value);
                                    }
                                }
                                if(isset($this->CustomerFormat["Style_Format"][$row_key]))
                                {
                                    if(is_callable($this->CustomerFormat["Style_Format"][$row_key]))
                                    {
                                        $style = $this->getStyle($currentColumn . $startRow);
                                        if(is_object($style))
                                        $this->CustomerFormat["Style_Format"][$row_key]($style);
                    ...
                    ++$currentColumn;
                    $rowData_key = array_keys($rowData);
                    if(end($rowData_key)!=  $row_key)
                    {
                        $end_column = $currentColumn;
                    }
                ...
                if($CustomerFormat)
                {
                    if(isset($CustomerFormat["Fill_Color"]) && $CustomerFormat["Fill_Color"]!="")
                    {
                        $row_string = "$startColumn"."$startRow".":".($end_column)."$startRow";
                        $this->getStyle($row_string)->applyFromArray($CustomerFormat["Fill_Color"]);
                    }
                    if(isset($CustomerFormat["Functions"]) && is_callable($CustomerFormat["Functions"]))
                    {
                        $CustomerFormat["Functions"]();   
                    }
                }
                /* End Here */
                ++$startRow;
                    ...
                    if ($cell->getValue() !== null) {
                        ...
                        if ($formatData) {
                            ...
                            $returnValue[$rRef][$cRef] = PHPExcel_Style_NumberFormat::toFormattedString(
								...
								isset($this->column_custom_format[(string)$cRef]) && is_callable($this->column_custom_format[(string)$cRef])?$this->column_custom_format[(string)$cRef]($rRef,$style):(($style && $style->getNumberFormat()) ?
     ...
     * @return array
     */
	public function toArray($nullValue = null, $calculateFormulas = true, $formatData = true, $returnCellRef = false,$rangeString=NULL)
    {
        // Garbage collect...
        $this->garbageCollect();
        ...
        if($rangeString==NULL)
        {
            ...
            $rangeString = 'A1:'.$maxCol.$maxRow;
        }

        // Return
		return $this->rangeToArray($rangeString,
									$nullValue, $calculateFormulas, $formatData, $returnCellRef);
    }
