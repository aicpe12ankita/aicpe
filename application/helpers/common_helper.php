...
*/
	...
	function currency_symbol_icon($currency_code='RS')
	{
		$currency_code = ($currency_code!='') ? strtoupper($currency_code) :'RS';
	...
	*/
	...
	function insert_system_scheduler($filter_array=array(),&$job_id=NULL)
	{
		if(!empty($filter_array))
		{
			...
			return $CI->Common_model->insert_all('system_scheduler_master', $filter_array);
		}
	}
    ...
    */
    function write_excel($file_path, $header = array(), $data, $ActiveSheetTitle = 'Sheet1', $data_before_header = array(), $custom_settings = array())
    {
    	...
    	try
    	{
    		...
    		if(isset($custom_settings['set_as_image']) && is_array($custom_settings['set_as_image']))
    		{
    			foreach($custom_settings['set_as_image'] as $header_label) 
    			{
    				if(isset($header))
    				{
    					if(in_array($header_label, $header))
    					{
    						$image_keys[] = array_search($header_label, $header);
    					}
    				}
    			}
    		}
    				...
    				else
    				{
    					foreach($row_data as $key => $column_value)
    					{
    						if(is_numeric($key))
    						{
    							...
    							elseif(in_array($key, $image_keys))
    							{	
    								$objPHPExcel->getActiveSheet()->getRowDimension($row_count)->setRowHeight(40);
    								$objPHPExcel->getActiveSheet()->getColumnDimension(get_letters_to_key($key))->setAutoSize(false);
									$objPHPExcel->getActiveSheet()->getColumnDimension("A")->setWidth("200");
    								...
    								$objDrawing->setCoordinates(get_letters_to_key($key).$row_count);
...
{	
 ...
 /*
    ...
    */
    ...
    /*
    @Description: Function to get number of working days by start date and end date
    @Author     : Bhavesh Chaudhari
    @Input      : 
    @Output     :
    @Date       : 18-01-2021
