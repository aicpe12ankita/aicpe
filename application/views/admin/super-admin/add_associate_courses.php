 ...
 <div class="modal-dialog" role="document">
          <div class="modal-content">
              ...
              <div class="modal-header custom-modal">
                  <h5 class="modal-title white"><?= isset($id) && $id!='' ? 'Edit Associate Course':'Add Associate Course' ?></h5>
              ...
              <div class="modal-body">
                <?= isset($id) && $id!='' ? '<form method="post" enctype="multipart/form-data" id="edit_assciate_course" onsubmit="return false;">' : '<form method="post" enctype="multipart/form-data" id="add_assciate_course" onsubmit="return false;">' ?>
                
                  <div class="form-group">
                      <label>Course Code</label>
                      <input type="text" class="form-control" name="crs_code" value="<?= isset($id) && $id!='' ? $data->asc_code : '' ?>" required>
                  </div>
                  <div class="form-group">
                      <label>Course Name</label>
                      <input type="text" class="form-control" name="crs_name" value="<?= isset($id) && $id!='' ? $data->asc_name : '' ?>" required>
                  </div>
                  <div class="form-group">
                      <label>Course Duration</label>
                      <input type="text" class="form-control" name="crs_duration" value="<?= isset($id) && $id!='' ? $data->asc_duration : '' ?>">
                  </div>
                  <div class="form-group">
                      ...
                      <select class="form-control select2-single" name="crs_stream" required>
                        ...
                        <?php if($streams!=""){
                          foreach ($streams as $stream) { ?>
                            <option value="<?php echo $stream->ct_id; ?>" <?php if(isset($id) && $id!=''){ if($data->asc_stream==$stream->ct_id){ echo "selected"; }} ?>><?php echo $stream->ct_title; ?></option>
                          <?php } } ?>
                      </select>
                  </div>
                  ...
                  <div class="form-group">
                     ...
                     <textarea class="form-control" name="crs_awards" id="editor03"><?= isset($id) && $id!='' ? $data->asc_awards : '' ?></textarea>
                  </div>
                  <div class="form-group">
                      <label>Eligibility</label>
                      <textarea class="form-control" name="crs_eligibility" id="editor02"><?= isset($id) && $id!='' ? $data->asc_eligibility : '' ?></textarea>
                  </div>
                  <div class="form-group">
                      ...
                      <div class="col-lg-4 col-xs-4">
                        <div class="custom-control custom-radio">
                            <input type="radio" id="semester" name="crs_pattern"
                                class="custom-control-input" value="0" <?php if(isset($id) && $id!=''){ if($data->asc_pattern==0){ echo "checked"; }} ?>>
                      ...
                      <div class="col-lg-4 col-xs-4">
                        <div class="custom-control custom-radio">
                          <input type="radio" id="yearly" name="crs_pattern"
                              class="custom-control-input" value="1" <?php if(isset($id) && $id!=''){ if($data->asc_pattern==1){ echo "checked"; }} ?>>
                  ...
                  <div class="form-group mb-1" id="anni_type">
                    <label> No. of Semester</label> 
                      <input type="number" class="form-control" placeholder="" name="crs_no_of_semester" value="<?= isset($id) && $id!='' ? $data->asc_no_of_semester : '' ?>">
                  </div>
                  <div class="form-group">
                      <label>No. Of Marksheeks </label>
                      <input type="number" class="form-control" name="crs_no_of_marksheets" value="<?= isset($id) && $id!='' ? $data->asc_no_of_marksheets : '' ?>">
                  </div>
                  <div class="form-group">
                      <label>Demo Exams Count</label>
                      <input type="number" class="form-control" name="crs_demo_exam_count" value="<?= isset($id) && $id!='' ? $data->asc_demo_exam_count : '' ?>">
                  </div>
                  <div class="form-group">
                      <label>AICPE Exam Fees</label>
                      <input type="text" class="form-control" name="crs_exam_fees" value="<?= isset($id) && $id!='' ? $data->asc_aicpe_exam_fees : '' ?>">
                  </div>
                  <div class="form-group">
                      <label>Associate Exam Fees</label>
                      <input type="text" class="form-control" name="asso_exam_fees" value="<?= isset($id) && $id!='' ? $data->asc_asso_exam_fees : '' ?>">
                  </div>
                  <div class="form-group">
                      <label>Marksheet Fees</label>
                      <input type="text" class="form-control" name="crs_marksheet_fees" value="<?= isset($id) && $id!='' ? $data->asc_marksheet_fees : '' ?>">
                  </div>
                  <div class="form-group">
                     <label>Course Syllabus</label>
                    <textarea name="crs_syllabus" id="editor01" class="form-control"><?= isset($id) && $id!='' ? $data->asc_syllabus : '' ?></textarea>
                  </div>
                  <div class="form-group">
                    <label>Subjects and Weightage</label>
                     <div class="input_fields_wrap">
                        ...
                        <?php if(isset($data) && $data!=""){ $i=0; 
                          $subs = json_decode($data->asc_subjects_weightage,true);
                  ...
                  <div class="form-group">
                      ...
                      <select class="form-control select2-multiple" name="crs_associates[]" multiple required>
                        <option>&nbsp;</option>
                        <?php if($associates!=""){ 
                          foreach ($associates as $associate) { ?>
                            <option value="<?php echo $associate->asso_id; ?>" <?php if(isset($id) && $id!=""){ if(in_array($associate->asso_id, json_decode($data->asc_associates))){ echo "selected"; }} ?> ><?php echo $associate->asso_name; ?></option>
                          <?php } } ?>
                      </select>
                  </div>
                  <div class="form-group">
                      ...
                      <select class="form-control select2-multiple" multiple name="crs_publish_for[]">
                        ...
                        <?php if($institutions!=""){
                          foreach ($institutions as $institution) { ?>
                            <option value="<?php echo $institution->id; ?>" <?php if($id!=""){ if( in_array($institution->id, json_decode($data->asc_publish_for))){ echo "selected"; }} ?>><?php echo $institution->inst_name; ?></option>
                          <?php } } ?>
                      </select>
                  </div>
                  <div class="form-group">
                    <label>Publish Date</label>
                    <input type="text" class="form-control datepicker" name="crs_publish_date" value="<?= isset($id) && $id!='' ? date('m/d/Y',strtotime($data->asc_publish_date)) : '' ?>">
                  </div>
                  <div class="form-group">
                    <label>Status </label>
                    <div class="row">
                      <div class="col-lg-4 col-xs-4">
                        <div class="custom-control custom-radio">
                            <input type="radio" id="Active" name="crs_status"
                                class="custom-control-input" value="0" <?php if(isset($id) && $id!=""){ if($data->asc_status==0){ echo "checked"; }} ?>>
                      ...
                      <div class="col-lg-4 col-xs-4">
                        <div class="custom-control custom-radio">
                          <input type="radio" id="Inactive" name="crs_status"
                              class="custom-control-input" value="1" <?php if(isset($id) && $id!=""){ if($data->asc_status==1){ echo "checked"; }} ?>>
                          <label class="custom-control-label" for="Inactive">Inactive</label>
                        </div>
                      </div>
                    </div>
                  </div>
                  <input type="hidden" name="id" value="<?= isset($data) && $data!="" ? $data->asc_id : "" ?>">
                  <input type="hidden" id="pattern" value="<?= isset($data) && $data!="" ? $data->asc_pattern : "" ?>">
     ...
     $('#add_assciate_course').validate({
      ...
      submitHandler: function(form){
        var formdata = new FormData(form);
          ...
          $.ajax({
            url: '<?php echo base_url(); ?>add-associate-courses',
            type:'post',
            dataType : 'json',
            data:formdata,
            processData: false,
  ...
  $(document).ready(function(){
      ...
      submitHandler: function(form){
        var formdata = new FormData(form);
          ...
          $.ajax({
            url: '<?php echo base_url(); ?>edit-associate-courses',
            type:'post',
            dataType : 'json',
            data:formdata,
            processData: false,
