<?php $this->load->view('admin/includes/admin_header'); ?>
    ...
    <main>
        <div class="row">
            <div class="col-12">
                <h1>AICPE Associates</h1>
                <div class="text-zero top-right-button-container">
                  <button type="button" class="btn btn-primary btn-lg top-right-button mr-1" onclick="get_associate_byId('','');">Add New Associate</button>
        ...
        <div class="container-fluid">
            <div class="card">
                ...
                <div  id="ajax_data">
                 <?php $this->load->view('admin/super-admin/aicpe_associates_ajax'); ?>
                </div>
              </div>
            </div>
        </div>
  ...
  <div class="modal fade" id="viewAssociate" tabindex="-1" role="dialog" aria-labelledby="viewAssociate" aria-hidden="true">
      <?php $this->load->view('admin/super-admin/view_aicpe_associate'); ?>
  </div>
...
<div class="modal fade modal-right" id="addAssociate" tabindex="-1" role="dialog" aria-labelledby="addAssociate" aria-hidden="true">
     <?php $this->load->view('admin/super-admin/add_aicpe_associate'); ?>
</div>
...
<script type="text/javascript">
  function get_associate_byId(type,id){
       
    $.ajax({
      url     : '<?php echo base_url(); ?>get-aicpe-associates',
      type    : 'POST',
      dataType  : 'html',
 ...
 function delete_associate_byId(id){ 
    $.ajax({
              url: '<?php echo base_url(); ?>delete-aicpe-associates',
              type:'post',
              data: {id:id},
              dataType: 'json',
              beforeSend: function()
              ...
              success: function(response){
                  if(response==true){
                    toastr.success('Associate has been deleted successfully','Success',{timeOut:5000});
 ...
 function change_asso_status(id){
  ...
  function view_associate_byId(id){
       
    $.ajax({
      url     : '<?php echo base_url(); ?>view-aicpe-associates',
      type    : 'POST',
      dataType  : 'html',
      data    : {id: id},
      beforeSend: function(){
      ...
      success:function(data){
        $('#viewAssociate').html(data);
        $('#viewAssociate').modal('show');
      }
    })
    .done(function() {
      $(".main-loader").hide();
