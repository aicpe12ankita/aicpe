<?php $this->load->view('admin/includes/admin_header'); ?>
    ...
    <main>
        <div class="row">
            <div class="col-12">
                <h1>AICPE Associate Courses</h1>
                <div class="text-zero top-right-button-container">
                  <button type="button" class="btn btn-primary btn-lg top-right-button mr-1" onclick="get_associatecourse_byId('','');">Add Associate Course</button>
      ...
      <div class="container-fluid">
          <div class="card">
              ...
              <div  id="ajax_data">
               <?php $this->load->view('admin/super-admin/associate_courses_ajax.php'); ?>
              </div>
            </div>
          </div>
      </div>
...
<div class="modal fade modal-right" id="addAssociateCourse" tabindex="-1" role="dialog" aria-labelledby="addAssociateCourse" aria-hidden="true">
     <?php $this->load->view('admin/super-admin/add_associate_courses'); ?>
</div>
  ...
  var base = $('#base_url').val();
    ...
    //SORTING
      ...
      $("body").on("click",'#reset-btn',function(){
       ...
       window.location.href='<?php echo base_url(); ?>admin-associate-courses';
        return false;
...
<script type="text/javascript">
  function get_associatecourse_byId(type,id){ 
       
    $.ajax({
      url     : '<?php echo base_url(); ?>get-associate-courses',
      type    : 'POST',
      dataType  : 'html',
      ...
      success:function(data){
        $('#addAssociateCourse').html(data);
        $('#addAssociateCourse').modal('show');
      }
    })
    .done(function() {
      $(".main-loader").hide();
 ...
 function delete_associatecourse_byId(id){ 
    $.ajax({
              url: '<?php echo base_url(); ?>delete-associate-courses',
              type:'post',
              dataType: 'json',
              data: {id:id},
              ...
              success: function(response){
                  if(response==true){
                    toastr.success('Associate course has been deleted successfully','Success',{timeOut:5000});
 ...
 function change_asso_status(id){
  
      $.ajax({
      url     : '<?php echo base_url(); ?>block-aicpe-associates',
      type    : 'POST',
      dataType  : 'json',
      data    : {id: id},
      beforeSend: function(){
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $.ajax({
      url:" <?php echo base_url().'export-associte-courses'; ?>",
      type: "POST",
      dataType : "json",
      data:{ 
