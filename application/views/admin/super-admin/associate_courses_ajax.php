                                   ...
                                   <?php if($data!=""){ $i=$from_records;
                                    foreach ($data as $row) { ?>
                                      ...
                                      <tr>
                                        ...
                                        <td>
                                            <p class="text-muted"><?php echo $row->asc_code; ?></p>
                                        </td>
                                        <td>
                                            <p class="text-muted"><?php echo $row->asc_name; ?></p>
                                        </td>
                                       <td>
                                           <?php
                                                if($associates!=""){
                                                    foreach ($associates as $associate) { 
                                                        if(in_array($associate->asso_id, json_decode($row->asc_associates))){ echo '<span class="badge badge-outline-pill badge-primary">'.$associate->asso_name.'</span> ' ; }
                                                    }
                                                } ?>
                                        </td>
                                        <td>
                                            <p class="text-muted"><?php echo $row->asc_duration; ?></p>
                                        </td>
                                        <td>
                                           <?php if($row->asc_subjects_weightage!=""){
                                            $subject_1 = array_keys(json_decode($row->asc_subjects_weightage, true)); 
                                                if($subjects!=""){
                                        ...
                                        <td>
                                         ...
                                         <td>
                                            <p class="text-muted"><?php echo date('d-m-Y', strtotime($row->asc_publish_date)); ?></p>
                                        </td>
                                        <td>
                                            <?php if($row->asc_status==0){
                                        ...
                                        <td>
                                            <a href="javascript:void(0);" class="btn btn-xs btn-primary mb-1" onclick="get_associatecourse_byId('edit','<?php echo $row->asc_id;?>');"><i class="simple-icon-note"></i></a>
                                            <a href="javascript:void(0);" class="btn btn-xs btn-warning mb-1"  onclick="delete_associatecourse_byId('<?php echo $row->asc_id;?>');"><i class="simple-icon-trash"></i></a>
