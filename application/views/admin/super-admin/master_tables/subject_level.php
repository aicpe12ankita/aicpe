    ...
    <main>
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-4">
                  <div id="form_data">
                    <?php $this->load->view('admin/super-admin/master_tables/add_subject_level'); ?>
                  </div>
                </div>
                <div class="col-lg-8">
                  <div class="card">
                    <div class="card-body">
                        ...
                        <div  id="ajax_data">
                         <?php $this->load->view('admin/super-admin/master_tables/subject_level_ajax.php'); ?>
                        </div> 
                      </div>
                  </div>
                </div>
...
<script type="text/javascript">
    ...
    var base = $('#base_url').val();
        function get_subject_level(type,id){
          $.ajax({
          url: base+'get-subject-level',
          type:'post',
          dataType:'html',
    ...
    //SORTING
        $("body").on("click",'a.sorting[sort-by]',function(e)
        {
         ...
         $("body").on("click",'#reset-btn',function(){
          window.location.href='<?php echo base_url(); ?>admin-subject-level';
          return false;
        });
  });
</script>
     ...
     rules:{
      ...
      submitHandler:function(form){
        ...
        $.ajax({
                  ...
                  success: function(response){
                      if(response==true){
                        toastr.success('New subject level has been added successfully','Success',{timeOut:5000});
...
<script type="text/javascript">
  function delete_subject_level(id){
    var new_id = id;

       $.ajax({
                  url: base+'delete-subject-level',
                  type:'post',
                  data: {new_id:new_id},
                  dataType: 'json',
                  beforeSend: function()
                  ...
                  success: function(response){
                      if(response==true){
                        toastr.success('Subject level has been deleted successfully','Success',{timeOut:5000});
